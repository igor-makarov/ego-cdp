...
- `ego-cdp status` - check if Chrome and WS daemon are running - allowed by sandbox, no escalation needed
- `ego-cdp start` - start Chrome and WS daemon (detects partial state and restarts if needed) - needs sandbox escalation
- `ego-cdp stop` - stop Chrome and WS daemon - needs sandbox escalation
...
`ego-cdp ws '<message>' [--timeout=ms] [--output=FILE]` - allowed by sandbox, no escalation needed

### Examples (non-exhaustive)

- `ego-cdp ws '{"id":1,"method":"Browser.getVersion"}'` - browser version
- `ego-cdp ws '{"id":1,"method":"Target.getTargets"}'` - list all tabs
- `ego-cdp ws '{"id":1,"method":"Target.createTarget","params":{"url":"<url>","background":true}}'` - create tab in background (no focus steal)
- `ego-cdp ws '{"id":1,"method":"Target.closeTarget","params":{"targetId":"<id>"}}'` - close a tab
- `ego-cdp ws '{"id":1,"method":"Target.attachToTarget","params":{"targetId":"<id>","flatten":true}}'` - attach to tab and get `sessionId`
- `ego-cdp ws '{"id":2,"sessionId":"<sessionId>","method":"Page.navigate","params":{"url":"<url>"}}'` - send target-scoped command using `sessionId`
- `ego-cdp ws '{"id":3,"sessionId":"<sessionId>","method":"Runtime.evaluate","params":{"expression":"document.title"}}'` - evaluate JS in a tab
