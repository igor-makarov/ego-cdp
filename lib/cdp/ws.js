...
/**
 * Send a CDP message over WebSocket via the persistent WS-pool daemon.
 * The daemon keeps connections open between CLI invocations.
 */
export async function runWsCommand({ path, message, timeout = 60000 }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    conn.on('connect', () => {
      conn.write(JSON.stringify({ path, message, timeout }) + '\n');
    });
