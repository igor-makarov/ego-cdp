...
function getExpectedResponseId(message) {
  try {
...
function sendOverCurrentWs(message, { timeout = 60000, expectedId = undefined } = {}) {
  const currentWs = ws;
...
async function sendKeepAlive() {
  ...
  const id = nextKeepAliveId--;
  const message = JSON.stringify({ id, method: 'Browser.getVersion' });

  try {
    await sendOverCurrentWs(message, {
...
async function executeRequest({ message, timeout = 60000 }) {
  ...
  return sendOverCurrentWs(normalizedMessage, {
    timeout,
    expectedId: getExpectedResponseId(normalizedMessage),
  });
}
