...
import { createChromeRemoteDebugAllowWatcher } from './chrome-remote-debug-allow.js';
...
const { values: args } = parseArgs({
  options: {
    ...
    'user-mode':            { type: 'boolean', default: false },
  },
});

const cdpPort = parseInt(args.port, 10);
const browserContextPath = args['browser-context-path'];
const userMode = args['user-mode'];

if (!Number.isInteger(cdpPort) || cdpPort <= 0 || !browserContextPath) {
  console.error('Usage: ws-daemon --port=PORT --browser-context-path=PATH [--user-mode]');
  process.exit(1);
}

const allowWatcher = createChromeRemoteDebugAllowWatcher({ enabled: userMode });
...
function sendOverCurrentWs(message, { timeout = 60000, expectedId = undefined, expectedSessionId = undefined } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      // Try to abort any running execution on the target to avoid bricking the page
...
function ensureWs() {
  ...
  // When ego-cdp is started in user mode, establishing the first DevTools
