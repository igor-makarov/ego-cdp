...
import { getChromeConfig, getUserDataDir } from '../common/config.js';
...
function buildArgs(userDataDir, { port, headless }) {
  const args = [
 ...
 */
function fetchBrowserSocketPath(port) {
...
export async function start() {
  ...
  const { headless, user, port } = getChromeConfig();
  ...
  if (user) {
    ...
    const child = spawn(CHROME, buildArgs(userDataDir, { port, headless }), {
      detached: true,
      stdio: 'ignore',
    });
    child.unref();
    ...
    cdpInfo = fetchBrowserSocketPath(port);
